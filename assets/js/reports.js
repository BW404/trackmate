...
async function loadReportData(period = 'today') {
    try {
        const response = await fetch(`api/get-activity-stats.php?period=${period}`);
        const result = await response.json();
        
        if (result.success && result.data) {
            updateReportStats(result.data, period);
            updateReportChart(result.data, period);
        }
    } catch (error) {
...
function updateReportStats(data, period) {
    // Update summary cards
    const workStat = data.summary.find(s => s.category === 2); // Working
    const phoneStat = data.summary.find(s => s.category === 1); // Phone
    
    // Total time
    const totalSeconds = data.summary.reduce((sum, stat) => sum + stat.time_seconds, 0);
    const totalHours = (totalSeconds / 3600).toFixed(1);
    document.querySelector('.summary-card:nth-child(1) .summary-value').textContent = totalHours + 'h';
    
    // Work time
    if (workStat) {
        document.querySelector('.summary-card:nth-child(2) .summary-value').textContent = workStat.hours + 'h';
    }
    
    // Phone time
    if (phoneStat) {
        document.querySelector('.summary-card:nth-child(3) .summary-value').textContent = phoneStat.hours + 'h';
    }
    
    // Productivity score
    document.querySelector('.summary-card:nth-child(4) .summary-value').textContent = data.productivity_score + '%';
}

// Update report chart
function updateReportChart(data, period) {
    // Implementation depends on Chart.js being loaded
    // This will create visualizations based on the period
}
...
document.addEventListener('DOMContentLoaded', () => {
    updateDate();
    initializeDates();
    loadReportData('today'); // Load today's data by default
